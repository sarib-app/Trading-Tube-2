...
const onShare = async () => {
  ...
  try {
    const result = await Share.share({
      message:
        `Hey there click here to register on tradingtube with my referal code URL https://registration.tradingtube.co/?${refer}`,
    });
...
<View style={styles.Header}>
    <Text
    onPress={()=> onShare()}
    style={styles.OuterTxt}>Weclcome{'\n'} <Text style={styles.InnerTxt}>{username}<Text style={{color:Colors.placeHolder,fontSize:12}}> Tap & share code {`( ${refer} )`}</Text></Text></Text>
